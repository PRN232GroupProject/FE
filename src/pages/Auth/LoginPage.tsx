...
const LoginPage: React.FC = () => {
  ...
  const onSubmit: SubmitHandler<LoginFormData> = async (data) => {
    ...
    try {
      ...
      if (!userFromToken) throw new Error('Invalid token');

      const finalUser = {
        ...userFromToken,
        role: apiRole || userFromToken.role,
      };

      loginToStore(token, finalUser);

      const roleCheck = String(finalUser.role).toLowerCase();
